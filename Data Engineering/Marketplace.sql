...
JOIN aerofleet.raw_pos.country c
...
SELECT
    ...
    ZEROIFNULL(SUM(odv.price)) AS daily_sales,
    ROUND(AVG(fd.avg_temperature_air_2m_f),2) AS avg_temperature_fahrenheit,
    ROUND(AVG(aerofleet.analytics.fahrenheit_to_celsius(fd.avg_temperature_air_2m_f)),2) AS avg_temperature_celsius,
    ROUND(AVG(fd.tot_precipitation_in),2) AS avg_precipitation_inches,
    ROUND(AVG(aerofleet.analytics.inch_to_millimeter(fd.tot_precipitation_in)),2) AS avg_precipitation_millimeters,
    MAX(fd.max_wind_speed_100m_mph) AS max_wind_speed_100m_mph
FROM aerofleet.harmonized.daily_weather_v fd
LEFT JOIN aerofleet.harmonized.orders_v odv
    ON fd.date_valid_std = DATE(odv.order_ts)
    AND fd.city_name = odv.primary_city
    AND fd.country_desc = odv.country
GROUP BY fd.date_valid_std, fd.city_name, fd.country_desc;
...
DROP VIEW IF EXISTS aerofleet.harmonized.daily_weather_v;

-- drop Analytics View
DROP VIEW IF EXISTS aerofleet.analytics.daily_city_metrics_v;

-- drop Fahrenheit to Celsius Function
DROP FUNCTION IF EXISTS aerofleet.analytics.fahrenheit_to_celsius(NUMBER(35,4));

-- drop Inch to Millimeter Function
DROP FUNCTION IF EXISTS aerofleet.analytics.inch_to_millimeter(NUMBER(35,4));
